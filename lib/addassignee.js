...
module.exports = class icpbot{
  ...
  async getsignoffIssues(){
    ...
    const Squadgroup=['sert','kubernetes','api','cam-integrations','catalog','cem-integration','cluster api opensource','core-services','gitops','grc-ui','helm-tiller','installer','istio','klusterlet','knative','metering','network','platform-ui','policy','power','search-analytics','security-audit','security-encryption','security-cert-mgmt','security-iam','server-app','server-foundation','service-broker','sre','storage','va','z','security-key-mgmt']
    ...
    this.logger.info(Squadgroup)
      for (var i in Squadgroup){

        var squadname=`squad:${Squadgroup[i]}`
    ...
    //issues.forEach(issue => this.github.issues.createComment({owner, repo,number: issue.number,body: signoffComment}))
    return //await issues.forEach(issue => this.github.issues.removeLabel({owner, repo, number: issue.number, name: 'DEV_SIGNOFF'}))
